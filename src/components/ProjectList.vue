<template>
  <header class="h-20 border-gray-100 border-b-2 flex items-center">
    <h1 class="w-1/6 text-xl font-bold pl-2">Projects</h1>
    <div class="w-3/6 h-1/2 flex items-center">
    ...
    <div class="w-2/6 h-1/2 flex items-center">
      <select class="w-2/3 max-w-full border border-gray-300 py-1 px-3 rounded-3xl">
  ...
  <article v-for="(item, index) in projects" :key="item.id" class="border-gray-100 border-b-2 flex flex-col h-32 px-3 py-2">
    <div class="h-1/2 flex items-center w-full">
      <h1 class="w-1/2 font-bold text-md">
        <a href="#" class="cursor-pointer hover:text-gray-500">{{ item.title }}</a>
      </h1>
      <div class="w-1/4 text-right">
        <a href="#" class="text-gray-500 hover:text-black">Visit site</a>
      </div>
      <div class="w-1/4 text-right pr-5">
    ...
    <div class="h-1/2 flex items-center w-full text-gray-500">
      <div class="w-1/2">
        <p><i class="fab fa-github fa-lg mr-2"></i>{{ user.id }}/{{ item.id }}</p>
      </div>
      <div class="w-1/2 pr-5 flex justify-end items-center">
